# Insane Blade·八刀 —— 快捷栏八刀概率参战（Forge 1.20.1）

主手之外，快捷栏其余八把拔刀剑以「PRD 保底 + 剑势连击」的概率引擎参与伤害计算；
摇中瞬间屏幕中下侧炸开刀身图标、叠加斩击音效。
与"拔刀剑：双刃重铸"完全独立：它管主手/副手/头盔三把的完整流程，本模组只管快捷栏其余八把。
...
## 四、配置项（存档 serverconfig/insaneblade-server.toml）
